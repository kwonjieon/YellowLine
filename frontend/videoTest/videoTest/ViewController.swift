...
//    let layerQueue = DispatchQueue(label: "sessionQueue")
...
extension ViewController {
    ...
    func upload() {
        ...
        let stringURL = "http://127.0.0.1:8000/yl/img"
...
//        }
        ...
        AF.upload(multipartFormData: { multiparfFormData in
            guard let image = UIImage(named: "chauchaudog.jpg"),
                  let imageData = image.jpegData(compressionQuality: 1) ?? image.pngData(),
                  let url = URL(string: stringURL)
            else {
            ...
            multiparfFormData.append(imageData, withName: "image", fileName: "\(image).png", mimeType: "image/png")
        }, to: stringURL, usingThreshold: UInt64.init(), method: .post, headers: header)
